...
/// A photo taken by the user during a Live Tour stop.
struct ProfilePhoto: Identifiable {
    let id: String          // photo_url (globally unique — used as stable identity)
    let url: String
    let neighborhood: String
}
...
final class ProfileViewModel: ObservableObject {
    ...
    @Published var allTours: [ProfileTour] = []  // every tour generated by this user
    @Published var favorites: [FavouriteSpot] = []
    @Published var photos: [ProfilePhoto] = []
    @Published var completedToursCount = 0
    @Published var totalStopsVisited = 0
    @Published var neighborhoodsExplored = 0
    ...
    func load(userId: String) async {
        ...
        // ── Tour history ──────────────────────────────────────────────────────
        // All tours this user has ever generated, newest first.
        struct HistoryRow: Codable {
            let id: String; let neighborhood: String; let stops: AnyCodable
            let share_token: String; let created_at: String; let tour_title: String?
        }
        let historyRows: [HistoryRow] = (try? await supabase.query(
            table: "tours", select: "id,neighborhood,stops,share_token,created_at,tour_title",
        ...
        allTours = historyRows.map { row in
            ...
            return ProfileTour(
                id: row.id, savedTourId: savedTourId,
                neighborhood: row.neighborhood, tourTitle: row.tour_title,
                stopCount: count, shareToken: row.share_token,
                savedAt: savedAt, customName: localNames[row.share_token]
            )
        }
        ...
        // ── Richer stats ──────────────────────────────────────────────────────
        // Count ALL visited_restaurants rows (not just favourites) for total stops
        // and distinct neighbourhood count.
        struct VisitRow: Codable { let place_id: String; let neighborhood: String? }
        let visitRows: [VisitRow] = (try? await supabase.query(
            table: "visited_restaurants",
            select: "place_id,neighborhood",
            filters: ["user_id": "eq.\(userId)"]
        )) ?? []
        totalStopsVisited = visitRows.count
        neighborhoodsExplored = Set(
            visitRows.compactMap(\.neighborhood).filter { !$0.isEmpty }
        ).count

        // ── Photo gallery ─────────────────────────────────────────────────────
        // Fetch all tours generated by this user so we can pull their stop photos.
        // tour_stop_photos has no direct user_id; link via tours.user_id.
        struct UserTourRow: Codable { let id: String; let neighborhood: String }
        let userTours: [UserTourRow] = (try? await supabase.query(
            table: "tours", select: "id,neighborhood",
            filters: ["user_id": "eq.\(userId)"],
            order: "created_at.desc"
        )) ?? []

        if !userTours.isEmpty {
            let tourNeighborhood = Dictionary(uniqueKeysWithValues: userTours.map { ($0.id, $0.neighborhood) })
            let idList = userTours.map(\.id).joined(separator: ",")

            struct PhotoRow: Codable { let tour_id: String; let photo_url: String }
            // PostgREST IN filter: ?tour_id=in.(id1,id2,...)
            if let photoRows: [PhotoRow] = try? await supabase.query(
                table: "tour_stop_photos",
                select: "tour_id,photo_url",
                filters: ["tour_id": "in.(\(idList))"]
            ) {
                photos = photoRows.map {
                    ProfilePhoto(
                        id: $0.photo_url,
                        url: $0.photo_url,
                        neighborhood: tourNeighborhood[$0.tour_id] ?? "Tour"
                    )
                }
            }
        }
